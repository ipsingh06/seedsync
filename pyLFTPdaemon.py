...
class json:
    def GET(self, filter):
        if filter == 'all':
            ...
            queuedFiles = self.__getFilteredFileData(web.ctx.pipe, status=[FileState.queued, FileState.downloading, FileState.downloaded])
...
FileState = enum(downloading='downloading', downloaded='downloaded', queued='queued', deleted='deleted', no_match='no_match')
...
class pyLFTP():
    ...
    def readConfig(self):
        ...
        logger.debug("localusername: %s, uid: %d, gid: %d"%(self.config["SSHConfig"]["localusername"], self.uid, self.gid))
	...
	logger.debug(commandIsfile)
        ...
        if len(lsOut) != len(lsOut2):
            logger.error("__getRemoteFileList: Output of similar du commands returned different outputs. lsOut: %d, lsOut2: %d" %(len(lsOut), len(lsOut2)))
            ...
            for i,line in enumerate(lsOut):
                ...
                if matchObj.group(2) != matchObj2.group(2):
                    logger.error("__getRemoteFileList: file names do not match: %s vs %s" % (matchObj.group(2), matchObj2.group(2)))
                    continue
    ...
    def __getLocalFileList(self):
        """
           ...
           Execute du command both with and without --apparent-size (-b) and take the smaller file size.
        """
        command = "cd %s; find * -maxdepth 0 | while read DIR; do find \"$DIR\" -type f -print0 | du -scB 1 --files0-from=- | tail -n 1 | sed \"s/total/$DIR/g\"; done" % self.config["SSHConfig"]["downloadtopath"]
        command2 = "cd %s; find * -maxdepth 0 | while read DIR; do find \"$DIR\" -type f -print0 | du -scb --files0-from=- | tail -n 1 | sed \"s/total/$DIR/g\"; done" % self.config["SSHConfig"]["downloadtopath"]
        lsOut, lsErr = subprocess.Popen(command, stdout=subprocess.PIPE, shell=True).communicate()
        lsOut = lsOut.splitlines()
        lsOut2, lsErr2 = subprocess.Popen(command2, stdout=subprocess.PIPE, shell=True).communicate()
        lsOut2 = lsOut2.splitlines()
        ...
        if len(lsOut) != len(lsOut2):
            logger.error("__getLocalFileList: Output of similar du commands returned different outputs. lsOut: %d, lsOut2: %d" %(len(lsOut), len(lsOut2)))
            logger.error(command)
            logger.error(command2)
            logger.error(lsOut)
            logger.error(lsOut2)
            ...
            for i,line in enumerate(lsOut):
                ...
                matchObj2 = re.search(r'(\d+)\s+(.*)', lsOut2[i].rstrip())
                if matchObj.group(2) != matchObj2.group(2):
                    logger.error("__getLocalFileList: file names do not match: %s vs %s" % (matchObj.group(2), matchObj2.group(2)))
                    continue
                fileName = matchObj.group(2)
                fileSize = min(int(matchObj.group(1)), int(matchObj2.group(1)))
                fileName = matchObj.group(2)
    ...
    def __startDownload(self, filename, filesize, isFile):
        ...
        self.process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, preexec_fn=self.__my_preexec_fn)
        ...
        def processOnExit(process,configDirectoryPath,filename,filesize):
            ...
            if localFilesize >= filesize:
                ...
                createFileProcess = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, preexec_fn=self.__my_preexec_fn)
                createFileProcess.wait()
                logger.info("Finished downloading %s" % filename)
                self.cleanRuntimeQueue(filename)
            return
        
        thread = threading.Thread(target=processOnExit, args=(self.process,self.configDirectoryPath,filename,filesize))
        thread.start()
        ...
        if buildType == BuildType.downloading:
            ...
            for (filename,filesize,isFile) in remoteFiles:
                ...
                if localFilesize:
                    ...
                    elif file.localSize > file.remoteSize and os.path.isfile("%s/_%s"%(self.configDirectoryPath,filename)):
